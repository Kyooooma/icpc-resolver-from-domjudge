...
class DOMjudge:
    ...
    def scoreboard_rank(self):
        for row in self.scoreboard['rows']:
            team_solved_func = lambda submission: submission['team_id'] == row['team_id'] and \
                                                  submission['judgement_type']['solved']
        ...
        self.scoreboard['rows'].sort(
            key=lambda x: (-x['score']['num_solved'], x['score']['total_time'], x['score']['max_submission_id']))
    ...
    def resolver_award_formatter(self):
        return reduce(lambda x, y: x + y, [
            ...
            self.resolver_award_top_team_formatter(int(self.config['award_rank'])),
    ...
    def resolver_award_first_solved_formatter(self):
        ...
        for submission in self.submissions:
            ...
            #     continue
            if ctime2timestamp(submission['contest_time']) >= ctime2timestamp(
                    self.contest_info['duration']) - ctime2timestamp(self.contest_info['scoreboard_freeze_duration']):
                continue
            idx = problem_id2idx[submission['problem_id']]
            if first_solved[idx]:
                continue
            first_solved[idx] = True
            first_solved_award.append(
                self.award('first-to-solve-%c' % chr(65 + idx), 'First to solve problem %c' % chr(65 + idx),
                           submission['team_id']))
    ...
    def resolver_award_top_team_formatter(self, rank):  # WARNING: 排名相同无法一起评
        ...
        for row in self.scoreboard['rows']:
            if cnt == rank:
                break
            if self.team_award_occupy(row['team_id']):  # 打星队伍不参与排名, 但参与评奖
                cnt += 1
            tot += 1
            buf.append([])
            buf[tot].append(row['team_id'])
        top_team_award = []
        cnt = 0
        for idx, team_ids in enumerate(buf):
            if len(team_ids) == 0:
                continue
            if not self.team_award_occupy(team_ids[0]):
                top_team_award.append(self.award(f'rank-{idx}', '打星队伍', team_ids))  # 打星队伍不参与排名
            else:
                cnt += 1
                top_team_award.append(self.award(f'rank-{idx}', '%s Place' % make_ordinal(cnt), team_ids))
    ...
    def resolver_award_winner_formatter(self):  # WARNING: 排名相同无法一起评
        ...
        for row in self.scoreboard['rows']:
            if cnt == rank:
                break
            if not self.team_award_occupy(row['team_id']):  # 打星队伍不参与排名
                continue
            cnt += 1
            buf[cnt].append(row['team_id'])
        winner_award = []
        for _, team_ids in enumerate(buf):
            winner_award.append(self.award(f'winner', 'Contest Champion', team_ids))
        return winner_award
    ...
    def resolver_award_best_girl_formatter(self):
        ...
        if best_girls_team_id != -1:
            best_girls_award.append(
                self.award(f"group-winner-{self.config['award_best_girl'][0]}", "The Best Girls's Team",
                           best_girls_team_id))
